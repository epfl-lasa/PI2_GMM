function [genCostSteps, totCostSteps, userCostSteps, R, viapointCostSteps, accelerationCostSteps, controlCostSteps, convergenceCost] = viapoint_LineTurnSyncOnlyFreeTime(D,GMR,p)
...
for k=1:n_rep,
    for m= 1:n_subtrials
      ...
      minVal =(D(k,m).q(1,20)-2).^2+(D(k,m).q(2,20)-0).^2;
      viapointCostSteps(20,k,m) = viapointCostSteps(20,k,m)+ viapointCostWeight* minVal;
      minVal =(D(k,m).q(1,30)-4).^2+(D(k,m).q(2,30)-0).^2;
      viapointCostSteps(30,k,m) = viapointCostSteps(30,k,m)+ viapointCostWeight* minVal;
      minVal =(D(k,m).q(1,45)-6).^2+(D(k,m).q(2,45)-0).^2;
      viapointCostSteps(45,k,m) = viapointCostSteps(45,k,m)+ viapointCostWeight* minVal;
      minVal =(D(k,m).q(1,65)-6).^2+(D(k,m).q(2,65)-2).^2;
      viapointCostSteps(65,k,m) = viapointCostSteps(65,k,m)+ viapointCostWeight* minVal;
      minVal =(D(k,m).q(1,80)-6).^2+(D(k,m).q(2,80)-4).^2;
      viapointCostSteps(80,k,m) = viapointCostSteps(80,k,m)+ viapointCostWeight* minVal;
     
        % reach goal free time
        minVal =(D(k,m).q(1,end)-6).^2+(D(k,m).q(2,end)-6).^2;
      viapointCostSteps(end,k,m) = viapointCostSteps(end,k,m)+ viapointCostWeight* minVal;
