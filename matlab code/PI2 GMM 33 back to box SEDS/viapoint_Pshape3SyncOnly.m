...
for k=1:n_rep,
    for m= 1:n_subtrials
      ...
      [minVal, Index] =  min((D(k,m).traj_y(1,1:n_steps_real)+45).^2+(D(k,m).traj_y(2,1:n_steps_real)-20).^2);
      viapointCostSteps(Index,k,m) = viapointCostSteps(Index,k,m)+ viapointCostWeight* minVal;
      [minVal, Index] =  min((D(k,m).traj_y(1,1:n_steps_real)+10).^2+(D(k,m).traj_y(2,1:n_steps_real)-30).^2);
      viapointCostSteps(Index,k,m) = viapointCostSteps(Index,k,m)+ viapointCostWeight* minVal;
      [minVal, Index] =  min((D(k,m).traj_y(1,1:n_steps_real)-10).^2+(D(k,m).traj_y(2,1:n_steps_real)-15).^2);
      viapointCostSteps(Index,k,m) = viapointCostSteps(Index,k,m)+ viapointCostWeight* minVal;
      
      % reach goal
      viapointCostSteps(end,k,m) = viapointCostSteps(end,k,m)+ viapointCostWeight*((D(k,m).traj_y(1,n_steps_real)-0).^2+(D(k,m).traj_y(2,n_steps_real)+0).^2);
