...
for k=1:n_rep,
    for m= 1:n_subtrials
      ...
      goalCost1 = viapointCostWeight*((D(k,m).q(1,n_steps_real)-10).^2+(D(k,m).q(2,n_steps_real)-5).^2) + 1;
      goalCost2 = viapointCostWeight*((D(k,m).q(1,n_steps_real)-11).^2+(D(k,m).q(2,n_steps_real)+10).^2) + 0;
