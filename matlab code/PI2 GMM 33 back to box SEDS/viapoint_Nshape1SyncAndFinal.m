function [genCostSteps, totCostSteps, userCostSteps, R, viapointCostSteps, accelerationCostSteps, controlCostSteps, convergenceCost] = viapointSEDS_Nshape1SyncAndFinal(D,GMR,p)
...
for k=1:n_rep,
    for m= 1:n_subtrials
      ...
      [minVal, Index] =  min((D(k,m).q(1,1:n_steps_real)+100).^2+(D(k,m).q(2,1:n_steps_real)+50).^2);
      viapointCostSteps(Index,k,m) = viapointCostSteps(Index,k,m)+ viapointCostWeight* minVal;
      [minVal, Index] =  min((D(k,m).q(1,1:n_steps_real)-0).^2+(D(k,m).q(2,1:n_steps_real)+70).^2);
      viapointCostSteps(Index,k,m) = viapointCostSteps(Index,k,m)+ viapointCostWeight* minVal;
      
      % reach goal
      viapointCostSteps(end,k,m) = viapointCostSteps(end-1,k,m)+ viapointCostWeight*((D(k,m).q(1,n_steps_real)-0).^2+(D(k,m).q(2,n_steps_real)+0).^2);
      
      
      % redundant final viapoint cost
       [minVal, Index] =  min((D(k,m).q(1,1:n_steps_real)+100).^2+(D(k,m).q(2,1:n_steps_real)+50).^2);
