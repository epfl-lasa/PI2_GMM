...
for k=1:n_rep,
    for m= 1:n_subtrials
      ...
      minVal  =  (D(k,m).q(1,10)+30).^2+(D(k,m).q(2,10)-100).^2;
      viapointCostSteps(10,k,m) = viapointCostSteps(10,k,m)+ viapointCostWeight* minVal;
      minVal =  (D(k,m).q(1,30)+20).^2+(D(k,m).q(2,30)-30).^2;
