function [genCostSteps, totCostSteps, userCostSteps, R, viapointCostSteps, accelerationCostSteps, controlCostSteps, convergenceCost] = viapoint_Pshape3SyncOnly(D,GMR,p)
...
for k=1:n_rep,
    for m= 1:n_subtrials
      ...
      % pass through one set of viapoints
      
      vp1ACost = viapointCostWeight*min(((D(k,m).q(1,:)+6).^2+(D(k,m).q(2,:)+3).^2));
      vp2ACost  = viapointCostWeight*min(((D(k,m).q(1,:)+4).^2+(D(k,m).q(2,:)-0).^2));
      vp3ACost  = viapointCostWeight*min(((D(k,m).q(1,:)+2).^2+(D(k,m).q(2,:)-3).^2));
     
      vp1BCost = viapointCostWeight*min(((D(k,m).q(1,:)-6).^2+(D(k,m).q(2,:)-0).^2));
      vp2BCost  = viapointCostWeight*min(((D(k,m).q(1,:)-5).^2+(D(k,m).q(2,:)+4).^2));
      vp3BCost  = viapointCostWeight*min(((D(k,m).q(1,:)-3).^2+(D(k,m).q(2,:)+4).^2));
      vp4BCost =  viapointCostWeight*min(((D(k,m).q(1,:)-2).^2+(D(k,m).q(2,:)+0).^2));
      
      goalCost = viapointCostWeight*((D(k,m).q(1,n_steps_real)-0).^2+(D(k,m).q(2,n_steps_real)-0).^2);
      
      viapointCostSteps(end,k,m) = min(vp1ACost + vp2ACost + vp3ACost, vp1BCost + vp2BCost + vp3BCost + vp4BCost) + goalCost;
