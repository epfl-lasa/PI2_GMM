...
for k=1:n_rep,
    for m= 1:n_subtrials
      ...
      [minVal, Index] =  min((D(k,m).q(1,1:n_steps_real)-1).^2+(D(k,m).q(2,1:n_steps_real)-0).^2);
      viapointCostSteps(Index,k,m) = viapointCostSteps(Index,k,m)+ viapointCostWeight* minVal;
      [minVal, Index] =  min((D(k,m).q(1,1:n_steps_real)-3).^2+(D(k,m).q(2,1:n_steps_real)-0).^2);
      viapointCostSteps(Index,k,m) = viapointCostSteps(Index,k,m)+ viapointCostWeight* minVal;
      [minVal, Index] =  min((D(k,m).q(1,1:n_steps_real)-5).^2+(D(k,m).q(2,1:n_steps_real)-3).^2);
      viapointCostSteps(Index,k,m) = viapointCostSteps(Index,k,m)+ viapointCostWeight* minVal;
