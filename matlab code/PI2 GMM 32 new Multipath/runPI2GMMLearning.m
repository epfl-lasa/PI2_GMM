...
% where the control polciy (or virtual trajectory) is represented by GMR instead of DMPs
% 
...
D.kp            = zeros(1, n_steps_tot);% gaind of the PD controller
...
if noisyStartPos
    startPosEps = randn(2,1,n_subtrials); 
else
    startPosEps = zeros(2,1,n_subtrials);
     startPosEps(:,:,1) = [-8;0];
     startPosEps(:,:,2) = [8;0];
%     startPosEps(:,:,3) = [-1;0];
end
...
for i=1:p.updates,
  ...
  D_eval=run_rollouts(D_eval,p_eval,1,startPosEps,FWindList);
 ...
 if i==1
  ...
  D=run_rollouts(D,p,noise_mult,startPosEps, FWindList);
  ...
  if (((mod(i,10)==1 && plotIntermediate) || i==p.updates) && ~p.disable_plotting)                  
    plotGraphs(D,totCostSteps,p,D_eval,totCostSteps_eval,p_eval, i*2)
  end
  
  % relearning GMM on best roll-outs if option selected (deprecated)
  if mod(i,20)== 1 && i~=1 && p.gaussian_centers == 1
       
        [v,inds]=sort(sum(totCostSteps,1));
        for j=1:p.rep,
          Dtemp = D(j);
          D(j) = D(inds(j));
          D(inds(j)) = Dtemp;
        end
        n_demos = size(eliteRolloutsTank.D,2);
        trainingData = zeros(5,n_demos*n_steps);
        for j = 1:n_demos
           % trainingData(:,(j-1)*(n_steps)+1:j*(n_steps)) = [D(j).q(1,:);
           % D(j).q(2,:); D(j).qd(1,:); D(j).qd(2,:); D(j).kp];  using
           % actual position as traning data produces a very slow motion
           % after the relearning of the GMR.
            trainingData(:,(j-1)*(n_steps)+1:j*(n_steps)) = [ eliteRolloutsTank.D(j).traj_y(1,:);  eliteRolloutsTank.D(j).traj_y(2,:);  eliteRolloutsTank.D(j).traj_yd(1,:);  eliteRolloutsTank.D(j).traj_yd(2,:);  eliteRolloutsTank.D(j).kp];
        end
        [Priors0, Mu0, Sigma0] = EM_init_kmeans(trainingData, n_Gauss);
        [GMM.Priors, GMM.Mu, GMM.Sigma, Pix] = EM(trainingData, Priors0, Mu0, Sigma0);
        computeGMM2GMR();
        
        % perform one noiseless evaluation to check new GMR
        D_eval_newGMR =run_rollouts(D_eval_newGMR,p_eval,1,startPosEps,FWindList);
        eval(sprintf('[genCostSteps_eval_newGMR, totCostSteps_eval_newGMR, userCostSteps_eval_newGMR]=%s(D_eval_newGMR,GMR);',p_eval.cost));
        figure(7*i);
        hold on
        plot(D_eval_newGMR.q(1,:),D_eval_newGMR.q(2,:),'b.');
        plot(D_eval_newGMR.traj_y(1,:),D_eval_newGMR.traj_y(2,:),'r.');
        for j = 1:n_demos
            plot(D(j).traj_y(1,:),D(j).traj_y(2,:),'y.'); 
            plot(D(j).q(1,:),D(j).q(2,:),'g.'); 
        end
        xlabel('q_1 & y_1')
        ylabel('q_2 & y_2');
        title('2D path of point mass & desired traj for relearning GMM');
        ...
        hold off
  elseif mod(i,10)== 1 && i~=1 && p.gaussian_centers == 3
      % decide which Gaussian to kill
      % place new Gaussian at center with high variance
      % compute likelyhood of each gaussian for each point
      % use this to compute mu and sigma
      computeGMR2GMM();
      killNspawnGaussian();
      computeGMM2GMR();
  else
  ...
  for j=1:n_Gauss
     ...
     if p.SEDS_constr && sum(GMR.A(1:n_dim,:,j)*[0;0]+GMR.b(1:n_dim,j) ~= [0;0]);
...
%-------------------------------------------------------------------------------
function D=run_rollouts(D,p,noise_mult, varargin)
...
if (D(1).u(1,1) == 0) % indicates very first batch of rollouts 
  start = 1;
    ...
    for k=start:p.rep,
      ...
      std_eps = computeStdRatio(p.std,GMR,0) * noise_mult;  % ratio tune
     %  std_eps =  p.std * noise_mult;                          % untuned
      ...
      if p.SEDS_constr                             % noise for kp is normally outside of here
          for j = 1:n_Gauss
              temp = randn(2,2).*std_eps;             % building symetric part d*S and trying decreasing d until (A+A')/2 + d*S is neg def
              ...
              temp = randn(2,2).*std_eps;           % building random assymetric part (no conditions on this part)
              temp = triu(temp,1);
              AS = temp - temp';                  
              epsA(:,:,j) = epsA(:,:,j) + AS;
...
%                   'problem with noise'
%               end
              epsB(:,j) = -epsA(:,:,j)*[0;0]; % goal is assumed to be 0,0
              
              epsInit((j-1)*(n_param/n_Gauss)+1:(j-1)*(n_param/n_Gauss)+n_dim^2) = reshape(epsA(:,:,j)',n_dim^2,1);
              epsInit((j-1)*(n_param/n_Gauss)+n_dim^2+n_in+1:(j-1)*(n_param/n_Gauss)+n_dim^2+n_in+n_dim) = epsB(:,j);   % the n_in is there for the rows of kp params
...
%               end
      ...
      if p.gaussian_centers == 2 
         epsInputMu = randn(n_in,n_Gauss).*repmat([3 3]', 1, n_Gauss).*std_eps;
         epsInputSigma = randn(n_in,n_in,n_Gauss).*std_eps;
         for j = 1:n_Gauss
            epsInputSigma(:,:,j) = triu(epsInputSigma(:,:,j)) + triu(epsInputSigma(:,:,j),1)';
            %epsInputSigma(:,:,j) = epsInputSigma(:,:,j)*epsInputSigma(:,:,j)';
            %probably not gaussian. makes sigma always grow
         end
         GMM.Mu(1:n_in,:) = GMM.Mu(1:n_in,:) + epsInputMu;
         GMM.Sigma(1:n_in,1:n_in,:) = GMM.Sigma(1:n_in,1:n_in,:)  + epsInputSigma;   % GMM is perturbed and GMR recomputed to ease the computation of the resulting G matrix (nonlinear effects). The initial value will be restored later before going to the next roll-out.
         for j = 1:n_Gauss
             [~,psdfCheck] = chol(GMM.Sigma(1:n_in,1:n_in,j));
             if psdfCheck
                  GMM.Sigma(1:n_in,1:n_in,j) = GMM.Sigma(1:n_in,1:n_in,j)  - epsInputSigma(:,:,j);
             end
             [~,psdfCheck] = chol(GMM.Sigma(1:n_in,1:n_in,j));
             if psdfCheck
                 ;
         ...
         computeGMM2GMR();
         D(k).epsInputMu = epsInputMu;
         D(k).epsInputSigma = epsInputSigma;
      end
        ...
        %       end
              ...
              %%%% uncomment for box scenario
%               if (q(1)> -2.5 && q(1)< -1.5 && q(2) < 2  && q(2) > 0) || (q(1)> -2.5 && q(1)< 2.5 && (q(2) > -2.5 && q(2) < -1.5 ))
%                   traj_yd = traj_yd*0;
%                   qd = qd*0;
%               end
...
%-------------------------------------------------------------------------------
function plotGraphs(D,totCostSteps,p,D_eval,totCostSteps_eval,p_eval, figID)
...
%   ylabel(sprintf('ydd_%d',j));
  ...
  % kp
  subplot(n_dim+1,4, n_dim*4+1);
  for k=1:p.rep,
      TT(:,k)=D(k).kp(:);
  end
  plot(T_tot,D_eval(1).kp(:),'Color',gray,'LineWidth',2);
  hold on;
  plot(T_tot,TT);
  hold off;
  ylabel(sprintf('kp'));
...
%showStreamlines(GMRreduced,currentAxis + 0.1*[currentAxis(1)-currentAxis(2) currentAxis(2)-currentAxis(1) currentAxis(3)-currentAxis(4) currentAxis(4)-currentAxis(3)],50);
showStreamlines(GMRreduced,[-10 10 -6 4],50);
