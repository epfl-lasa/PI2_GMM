function [genCostSteps, totCostSteps, userCostSteps, R, viapointCostSteps, accelerationCostSteps, controlCostSteps, convergenceCost] = costFunc_multiGoal2(D,GMR,p)
...
for k=1:n_rep,
    for m= 1:n_subtrials
      ...
      for n=1:n_steps_real,
        ...
        accelerationCostSteps(n,k)  =  QQ * norm(D(k,m).qd(:,n));
      end

      
      % reach  one of the goals
      
      goalCost1 = viapointCostWeight*((D(k,m).q(1,n_steps_real)-10).^2+(D(k,m).q(2,n_steps_real)-8.5).^2) + 1;
      goalCost2 = viapointCostWeight*((D(k,m).q(1,n_steps_real)-8).^2+(D(k,m).q(2,n_steps_real)+6).^2) + 0;
      
      
        viapointCostSteps(end,k,m) = min(goalCost1, goalCost2);
