function [genCostSteps, totCostSteps, userCostSteps, R, viapointCostSteps, accelerationCostSteps, controlCostSteps, convergenceCost] = costFunc_multiGoal2VP(D,GMR,p)
...
accelerationCostSteps = zeros(n_steps_real, n_rep, n_subtrials);  % is actually velocity cost
...
for k=1:n_rep,
    for m= 1:n_subtrials
      ...
      for n=1:n_steps_real,
        ...
        accelerationCostSteps(n,k)  =  QQ * norm(D(k,m).qdd(:,n));
      end

      
      % reach  one of the goals
      
      vp1Cost = viapointCostWeight*min(((D(k,m).q(1,:)-8).^2+(D(k,m).q(2,:)-8).^2)) + 0.5;
      vp2Cost = viapointCostWeight*min(((D(k,m).q(1,:)-8).^2+(D(k,m).q(2,:)+10).^2)) + 0;
      
      goalCost1 = viapointCostWeight*((D(k,m).q(1,n_steps_real)-12).^2+(D(k,m).q(2,n_steps_real)-6).^2) + 0.5;
      goalCost2 = viapointCostWeight*((D(k,m).q(1,n_steps_real)-11).^2+(D(k,m).q(2,n_steps_real)+7).^2) + 0;
      
      
        viapointCostSteps(end,k,m) = min(goalCost1, goalCost2) + min(vp1Cost,vp2Cost);
