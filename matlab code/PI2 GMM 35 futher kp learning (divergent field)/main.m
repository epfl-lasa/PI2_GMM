    ...
    if useSEDS
        ...
        for cnt2 = 1:n_demos
         n_steps = size(demos{cnt2},2);
            ...
            %trainingData(:,sum_steps - n_steps+1 +1:sum_steps) = [demos{cnt2}(1,1:n_steps-1); demos{cnt2}(2,1:n_steps-1); demos{cnt2}(3,1:n_steps-1); diff(demos{cnt2}(1,:))/dt; diff(demos{cnt2}(2,:)/dt) ; diff(demos{cnt2}(3,:)/dt); kp0*ones(1,n_steps-1)];
