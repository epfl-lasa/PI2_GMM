...
p = readProtocol('protocol_test.txt');

for cnt1 = 1:length(p)
    random_number_generator_state = rng; % stored for the purpose of reproducing the exact same experiment if needed.
   ...
   % dt = 0.1;  %%% !!!!!!!!!!!!!!!!!!!!!!remove
    ...
    if useSEDS
        ...
        for cnt2 = 1:n_demos
            ...
            %trainingData(:,sum_steps - n_steps+1 +1:sum_steps) = [demos{cnt2}(1,1:n_steps-1); demos{cnt2}(2,1:n_steps-1); demos{cnt2}(3,1:n_steps-1); diff(demos{cnt2}(1,:))/dt; diff(demos{cnt2}(2,:)/dt) ; diff(demos{cnt2}(3,:)/dt)];
        end
        ...
        for cnt2 = 1:n_demos
         n_steps = size(demos{cnt2},2);
            sum_steps = sum_steps + n_steps-1;
            trainingData(:,sum_steps - n_steps+1 +1:sum_steps) = [demos{cnt2}(1,1:n_steps-1); demos{cnt2}(2,1:n_steps-1); diff(demos{cnt2}(1,:))/dt; diff(demos{cnt2}(2,:)/dt) ; kp0*ones(n_dim_kp,n_steps-1)];
            %trainingData(:,sum_steps - n_steps+1 +1:sum_steps) = [demos{cnt2}(1,1:n_steps-1); demos{cnt2}(2,1:n_steps-1); demos{cnt2}(3,1:n_steps-1); diff(demos{cnt2}(1,:))/dt; diff(demos{cnt2}(2,:)/dt) ; diff(demos{cnt2}(3,:)/dt); kp0*ones(n_dim_kp,n_steps-1)];
