function [genCostSteps, totCostSteps, userCostSteps, R, viapointCostSteps, accelerationCostSteps, controlCostSteps, convergenceCost] = viapoint_Nshape3SyncOnly(D,GMR,p)
...
for k=1:n_rep,
    for m= 1:n_subtrials
      ...
      [minVal, Index] =  min((D(k,m).q(1,1:n_steps_real)+90).^2+(D(k,m).q(2,1:n_steps_real)+10).^2);
      viapointCostSteps(Index,k,m) = viapointCostSteps(Index,k,m)+ viapointCostWeight* minVal;
      [minVal, Index] =  min((D(k,m).q(1,1:n_steps_real)+60).^2+(D(k,m).q(2,1:n_steps_real)+130).^2);
      viapointCostSteps(Index,k,m) = viapointCostSteps(Index,k,m)+ viapointCostWeight* minVal;
