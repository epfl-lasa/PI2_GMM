function [genCostSteps, totCostSteps, userCostSteps, R, viapointCostSteps, accelerationCostSteps, controlCostSteps, stiffnessCostSteps] = costFunc_sineDivForceField(D,GMR,p)
...
QQ = 0.001;
R = 0.0000001*eye(length(GMR.theta));
StiffCost = 0.0002;
...
for k=1:n_rep,
    for m= 1:n_subtrials
      ...
      [minVal, Index] =  min((D(k,m).q(1,1:n_steps_real)+7.5).^2+(D(k,m).q(2,1:n_steps_real)+1).^2);
      viapointCostSteps(Index,k,m) = viapointCostSteps(Index,k,m)+ viapointCostWeight* minVal;
      [minVal, Index] =  min((D(k,m).q(1,1:n_steps_real)+5).^2+(D(k,m).q(2,1:n_steps_real)+0).^2);
      viapointCostSteps(Index,k,m) = viapointCostSteps(Index,k,m)+ viapointCostWeight* minVal;
      [minVal, Index] =  min((D(k,m).q(1,1:n_steps_real)+2.5).^2+(D(k,m).q(2,1:n_steps_real)-1).^2);
      viapointCostSteps(Index,k,m) = viapointCostSteps(Index,k,m)+ viapointCostWeight* minVal;
      [minVal, Index] =  min((D(k,m).q(1,1:n_steps_real)+1.25).^2+(D(k,m).q(2,1:n_steps_real)-0.5).^2);
      viapointCostSteps(Index,k,m) = viapointCostSteps(Index,k,m)+ viapointCostWeight* minVal;
