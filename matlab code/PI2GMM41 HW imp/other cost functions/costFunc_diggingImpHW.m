function [genCostSteps, totCostSteps, userCostSteps, R, viapointCostSteps, accelerationCostSteps, controlCostSteps, dirtCostSteps, forcesCostSteps, stiffCostSteps] = costFunc_diggingHW(D,GMR,p)
...
for k=1:n_rep,
    for m= 1:n_subtrials
      ...
      dirtCostSteps(end,k,m) = dirtCostWeight*(0.6*9.8 + mean(D(k,m).real_F(3,end-50:end-40)));  % 170 grams is the max capacity for the shovel (volume wise) / 550g for humide sand
