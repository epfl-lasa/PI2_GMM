...
convergenceCost = zeros(n_steps_convergence, n_rep, n_subtrials); % is actually velocity cost
avoidanceCost = zeros(n_steps_convergence, n_rep, n_subtrials);
for k=1:n_rep,
    for m= 1:n_subtrials
      ...
      for n=1:n_steps_real,
        ...
        convergenceCost(n,k)  =  norm(D(k,m).qd(:,n))*0.1;
      end
       convergenceCost(1,k)  =  convergenceCost(1,k);
      
      % reach the goal
       viapointCostSteps(end,k,m) = viapointCostWeight*((D(k,m).q(1,n_steps_real)-0).^2+(D(k,m).q(2,n_steps_real)-0).^2);
...
%--------------------------------------------------------------------------
userCostSteps = viapointCostSteps + accelerationCostSteps + convergenceCost;
