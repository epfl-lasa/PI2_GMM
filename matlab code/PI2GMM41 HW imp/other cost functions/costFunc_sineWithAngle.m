function [genCostSteps, totCostSteps, userCostSteps, R, viapointCostSteps, accelerationCostSteps, controlCostSteps, stiffnessCostSteps] = costFunc_sineWithAngle(D,GMR,p)
...
stiffnessCostSteps = zeros(n_steps_convergence, n_rep, n_subtrials);
for k=1:n_rep,
    for m= 1:n_subtrials
      ...
      for n=1:n_steps_real,
        ...
        if size(D(k,m).kp,1) == 1
            stiffnessCostSteps(n,k) = StiffCost * norm(repmat(D(k,m).kp(:,n),2,1),1);
        else
            stiffnessCostSteps(n,k) = StiffCost * norm(D(k,m).kp(:,n),1); % the one-norm of the stiffness. Does the 2-norm make more sens?
        end
      end
      ...
      [minVal, Index] =  min((D(k,m).q(1,1:n_steps_real)+7.5).^2+(D(k,m).q(2,1:n_steps_real)+1).^2 + (D(k,m).q(3,1:n_steps_real)+0).^2);
      viapointCostSteps(Index,k,m) = viapointCostSteps(Index,k,m)+ viapointCostWeight* minVal;
      [minVal, Index] =  min((D(k,m).q(1,1:n_steps_real)+5).^2+(D(k,m).q(2,1:n_steps_real)+0).^2 + (D(k,m).q(3,1:n_steps_real)+0).^2);
      viapointCostSteps(Index,k,m) = viapointCostSteps(Index,k,m)+ viapointCostWeight* minVal;
      [minVal, Index] =  min((D(k,m).q(1,1:n_steps_real)+2.5).^2+(D(k,m).q(2,1:n_steps_real)-1).^2 + (D(k,m).q(3,1:n_steps_real)+1).^2);
      viapointCostSteps(Index,k,m) = viapointCostSteps(Index,k,m)+ viapointCostWeight* minVal;
      [minVal, Index] =  min((D(k,m).q(1,1:n_steps_real)+2.5).^2+(D(k,m).q(2,1:n_steps_real)-1).^2 + (D(k,m).q(3,1:n_steps_real)+1).^2);
      viapointCostSteps(Index,k,m) = viapointCostSteps(Index,k,m)+ viapointCostWeight* minVal;
      [minVal, Index] =  min((D(k,m).q(1,1:n_steps_real)+1.25).^2+(D(k,m).q(2,1:n_steps_real)-0.5).^2 + (D(k,m).q(3,1:n_steps_real)+0).^2);
      viapointCostSteps(Index,k,m) = viapointCostSteps(Index,k,m)+ viapointCostWeight* minVal;
...
%--------------------------------------------------------------------------
userCostSteps = viapointCostSteps + accelerationCostSteps + stiffnessCostSteps ;
