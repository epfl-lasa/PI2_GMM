function [genCostSteps, totCostSteps, userCostSteps, R, viapointCostSteps, accelerationCostSteps, controlCostSteps, stiffnessCostSteps] = costFunc_lineWind(D,GMR,p)
...
QQ = 0.0001;
R = 0.000005*eye(length(GMR.theta));
    ...
    for m= 1:n_subtrials
      ...
      for n=1:n_steps_real,
        ...
        if size(D(k,m).kp,1) == 1
            stiffnessCostSteps(n,k) = 0.0005 * norm(repmat(D(k,m).kp(:,n),2,1),1);
        else
            stiffnessCostSteps(n,k) = 0.0005 * norm(D(k,m).kp(:,n),1); % the one-norm of the stiffness. Does the 2-norm make more sens?
        end
      end
      ...
      [minVal, Index] =  min((D(k,m).q(1,1:n_steps_real)+8).^2+(D(k,m).q(2,1:n_steps_real)+0).^2);
