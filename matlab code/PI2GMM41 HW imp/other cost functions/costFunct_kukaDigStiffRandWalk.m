function [genCostSteps, totCostSteps, userCostSteps, R, viapointCostSteps, accelerationCostSteps, controlCostSteps, stiffnessCostSteps] = viapointLineDigMultiPenalizeStiffness(D,GMR,p)
...
viapointCostWeight = 1000;
stifnessCostWeight = 0.000001;
...
for k=1:n_rep,
    for m= 1:n_subtrials
      ...
      for n=1:n_steps_real,
        ...
        stiffnessCostSteps(n,k)  = norm(D(k,m).kp(:,n))^2*stifnessCostWeight;
      end
      ...
      % pass through the viapoints
        ...
        [minVal Index] =  min((D(k,m).q(1,:)+0.2).^2+(D(k,m).q(2,:)+0.1).^2);
  viapointCostSteps (Index,k,m) = viapointCostSteps (Index,k,m) + minVal * viapointCostWeight;
  [minVal Index] =  min((D(k,m).q(1,:)+0.1).^2+(D(k,m).q(2,:)+0.15).^2);
 viapointCostSteps (Index,k,m) = viapointCostSteps (Index,k,m) + minVal * viapointCostWeight;
  [minVal Index] =  min((D(k,m).q(1,:)+0.05).^2+(D(k,m).q(2,:)+0.1).^2);
  viapointCostSteps (Index,k,m) = viapointCostSteps (Index,k,m) + minVal * viapointCostWeight;

  
  viapointCostSteps (end,k,m) = viapointCostSteps (end,k,m) + ((D(k,m).q(1,end)+0).^2+(D(k,m).q(2,end)+0).^2) * viapointCostWeight;
