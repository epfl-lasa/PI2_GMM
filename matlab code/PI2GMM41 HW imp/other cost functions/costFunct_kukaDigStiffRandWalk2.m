function [genCostSteps, totCostSteps, userCostSteps, R, viapointCostSteps, accelerationCostSteps, controlCostSteps, dirtCostSteps, forcesCostSteps, stiffnessCostSteps] = viapointLineDigMultiPenalizeStiffness(D,GMR,p)
...
QQ = 0.00001;
R = 0.000000000001*eye(length(GMR.theta));

viapointCostWeight = 10;
stiffnessCostWeight = 0.0000005;
...
for k=1:n_rep,
    for m= 1:n_subtrials
      ...
      for n=1:n_steps_real,
        ...
        accelerationCostSteps(n,k,m)  = 0.5 * QQ * norm(D(k,m).qdd(:,n))^2;
        stiffnessCostSteps(n,k,m)  = norm(D(k,m).kp(:,n))^2*stiffnessCostWeight;
      end
      ...
      % pass through the viapoints
        ...
        [minVal Index] =  min(sqrt((D(k,m).q(1,:)+0.2).^2+(D(k,m).q(2,:)+0.12).^2));
  viapointCostSteps (Index,k,m) = viapointCostSteps (Index,k,m) + minVal * viapointCostWeight;
  [minVal Index] =  min(sqrt((D(k,m).q(1,:)+0.1).^2+(D(k,m).q(2,:)+0.15).^2));
 viapointCostSteps (Index,k,m) = viapointCostSteps (Index,k,m) + minVal * viapointCostWeight;
  [minVal Index] =  min(sqrt((D(k,m).q(1,:)+0.05).^2+(D(k,m).q(2,:)+0.08).^2));
  viapointCostSteps (Index,k,m) = viapointCostSteps (Index,k,m) + minVal * viapointCostWeight;

  
  viapointCostSteps (end,k,m) = viapointCostSteps (end,k,m) + sqrt(((D(k,m).q(1,end)+0).^2+(D(k,m).q(2,end)+0).^2)) * viapointCostWeight;
