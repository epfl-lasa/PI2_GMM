function [genCostSteps, totCostSteps, userCostSteps, R, viapointCostSteps, accelerationCostSteps, controlCostSteps, angleCostSteps] = costFunc_diggingKukaSim(D,GMR,p)
...
angleCostSteps = zeros(n_steps_real, n_rep, n_subtrials);
...
for k=1:n_rep,
    for m= 1:n_subtrials
      ...
      for n=1:n_steps_real,
        ...
        if D(k,m).q(2,n) > -0.1
            angleCostSteps(n,k) = angleCostWeight*abs(D(k,m).q(3,n));   % angle should be 0 in second part
        else
            angleCostSteps(n,k) =  angleCostWeight*abs(D(k,m).q(3,n) + atan( D(k,m).qd(2,n)/D(k,m).qd(1,n))); % atan(...) is the negative of the value of the angle
        end
      end
      ...
      [minVal, Index] =  min((D(k,m).q(1,1:n_steps_real)+0.2).^2+(D(k,m).q(2,1:n_steps_real)+0.2).^2);
      viapointCostSteps(Index,k,m) = viapointCostSteps(Index,k,m)+ viapointCostWeight* minVal;
      [minVal, Index] =  min((D(k,m).q(1,1:n_steps_real)+0.1).^2+(D(k,m).q(2,1:n_steps_real)+0.2).^2);
      viapointCostSteps(Index,k,m) = viapointCostSteps(Index,k,m)+ viapointCostWeight* minVal;
...
angleCostSteps = mean(angleCostSteps,3);
...
%--------------------------------------------------------------------------
userCostSteps = viapointCostSteps + accelerationCostSteps + angleCostSteps ;
