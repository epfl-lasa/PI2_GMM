...
global n_param;
global n_dim_pos;         % number of dimension of the problem. Here we assume a 2D problem
global n_dim_angle;
global n_dim_kp;
global n_dim;
global outputFile;

% deffinition of dimensions of the problem and stiffness matrix shape
% (isotropic : n_dim_kp = 1, diagonal, n_dim_kp = n_dim)
n_dim_pos = 2;
n_dim_angle = 1;
...
% create the data matrix for the roll-out data
...
D.kp            = zeros(n_dim, n_steps_tot);% gaind of the PD controller

if p.HWinLoop
    D.real_q = zeros(n_dim, n_steps_tot);       % position received from robot
    D.real_F   = zeros(n_dim, n_steps_tot);     % sensed force received from robot
    D.real_T   = zeros(n_dim, n_steps_tot); 
end
...
% for reusing the best roll-outs
n_elite_tank = 10;
...
result.Ds(1:p.updates,1:p.rep,1:n_subtrials) = D;

D(1:p.rep,1:n_subtrials) = D;  % one data structure for each repetition

result.cost = zeros(p.updates+1,10);             % the cost with initial parameters is also recorded
result.thetas = zeros(p.updates+1,n_param);     % the initial parameters are also stored

% in case sub-trials are performed
if noisyStartPos
...
FWindList = zeros(n_dim_pos,n_subtrials);
%FWindList = [[0;0],[10;0]];
...
for i=1:p.updates,
  ...
  %%% compute evaluation cost cost 
  %eval(sprintf('[genCostSteps_eval, totCostSteps_eval, userCostSteps_eval, R]=%s(D_eval,GMR);',p_eval.cost));
  eval(sprintf('[genCostSteps_eval, totCostSteps_eval, userCostSteps_eval, R, viapointCostSteps_eval, accelerationCostSteps_eval, controlCostSteps_eval, dirtCostSteps_eval, forcesCostSteps_eval, stiffCostSteps_eval]=%s(D_eval,GMR,p);',p_eval.cost));
 ...
 %%% display cost
  ...
  %%% store evaluation cost
  %result.cost(i,:) = [i*(p.rep-p.n_reuse)+p.n_reuse - 1,[sum(genCostSteps_eval), sum(totCostSteps_eval), sum(userCostSteps_eval)]];
  result.cost(i,:) = [double((i-1)*(p.rep-p.n_reuse)+p.n_reuse),sum(genCostSteps_eval), sum(totCostSteps_eval), sum(userCostSteps_eval), sum(viapointCostSteps_eval), sum(accelerationCostSteps_eval), sum(controlCostSteps_eval), sum(dirtCostSteps_eval), sum(forcesCostSteps_eval), sum(stiffCostSteps_eval)];
  result.thetas(i,:) = GMR.theta';
  result.Ds_eval(i,:,:) = D_eval;
  

  %%% run roll-outs with a noise annealing multiplier. noise
  ...
  %%% Inject previous nominal polcy in roll-out batch for robustness
  D(1,:) = D_eval;
  
  result.Ds(i,:,:) = D;
  
  
  %%% Add elite roll-outs to the batch in case of reuse
  if (i > 1 && p.n_reuse > 0)
  ...
  %%% compute costs of roll-outs
  eval(sprintf('[genCostSteps, totCostSteps, userCostSteps, R, viapointCostSteps, accelerationCostSteps, controlCostSteps, dirtCostSteps, forcesCostSteps, stiffCostSteps]=%s(D,GMR,p);',p.cost));
  if ~p.disable_plotting
    figure;
    hold on
    plot(sum(totCostSteps(:,:,1),1));   % tot cost
    plot(sum(viapointCostSteps(:,:,1),1),'r'); % viapoint cost
    plot(sum(accelerationCostSteps(:,:,1),1),'g'); % acceleration cost
    plot(sum(controlCostSteps(:,:,1),1),'k'); % control cost
    plot(sum(dirtCostSteps(:,:,1),1),'c'); % dirt cost
    plot(sum(forcesCostSteps(:,:,1),1),'m'); % forces cost
    plot(sum(stiffCostSteps(:,:,1),1),'y'); % stiffness cost
    xlabel('Number of roll outs');
    ylabel('TotCost');
    hold off
end
  %result.rollouts(i,:,:) = D;
  %result.costRollouts(i,:,:,:) = [genCostSteps, totCostSteps, userCostSteps, viapointCostSteps, accelerationCostSteps, controlCostSteps];
  ...
  for j=1:n_Gauss
     ...
     [trash isNotNegDef] = chol(-(GMR.A(1:n_in,:,j)+GMR.A(1:n_in,:,j)')/2);
...
result.cost(p.updates+1,:) = [double((i-1)*(p.rep-p.n_reuse)+1+p.n_reuse),sum(genCostSteps_eval), sum(totCostSteps_eval), sum(userCostSteps_eval), sum(viapointCostSteps_eval), sum(accelerationCostSteps_eval), sum(controlCostSteps_eval), sum(dirtCostSteps_eval), sum(forcesCostSteps_eval), sum(stiffCostSteps_eval)];
...
if ~p.disable_plotting
    ...
    plot(result.cost(:,1),result.cost(:,8),'c'); % dirt cost
    plot(result.cost(:,1),result.cost(:,9),'m'); % forces cost
    plot(result.cost(:,1),result.cost(:,10),'y'); % stiffness cost
    xlabel('Number of roll outs');
    ylabel('TotCost');
    hold off
end
...
%%% run roll-outs
%startIndex = p.n_reuse + 1;
startIndex = 2;  % first roll-out is eval rollout from previous iteration
if (p.rep == 1) % || D(1).u(1,1) == 0) % indicates very first batch of rollouts 
  startIndex = 1;  
end
    k = startIndex;
    while k <= p.rep
      ...
      %epsInit = randn(n_param,1).*std_eps.*repmat([ 1*ones(1,n_dim_pos*n_dim_pos) 20*ones(1,n_dim_pos*n_dim_kp) 1*ones(1,n_dim_pos) 100*ones(1,n_dim_kp)]', n_Gauss, 1);  % compatible with multi-dim stiffness
        epsInit = randn(n_param,1).*std_eps.*repmat([ 1*ones(1,(n_in)*(n_dim_pos)) 1*ones(1,(n_in)*(n_dim_angle)) 500*ones(1,n_in*n_dim_kp) 1*ones(1,(n_dim_pos)) 1*ones(1,(n_dim_angle)) 2500*ones(1,n_dim_kp)]', n_Gauss, 1);
        %epsInit = randn(n_param,1).*std_eps.*repmat([ 0*ones(1,(n_in)*(n_dim_pos)) 0*ones(1,(n_in)*(n_dim_angle)) 500*ones(1,n_in*n_dim_kp) 0*ones(1,(n_dim_pos)) 0*ones(1,(n_dim_angle)) 2500*ones(1,n_dim_kp)]', n_Gauss, 1);
...
%               end
              ...
              epsInit((j-1)*(n_param/n_Gauss)+n_in*n_out+1:(j-1)*(n_param/n_Gauss)+n_in*n_out+(n_out-n_dim_kp)) = epsB(:,j);   % the n_in*n_dim_kp is there for the rows of kp params
...
%               end
        ...
        for m=1:n_subtrials
            ...
            %%% define perturbation for digging simulation
           % w = zeros(n_dim,1);
             w = 0.1*randn(n_dim,1);
          ...
          for n=1:n_steps_tot  
              ...
              if ~p.fixed_noise
               ...
               error('non-fixed noise option deprecated')
              end
        ...
        %%% define aera of perturbation for digging simulation
             perturbationForce = zeros(n_dim,1);
              if q(2)< -0.08
                  perturbationForce = w;
              end

              u   =   kp.*(traj_y-q) + kd.*(traj_yd-qd); % impedance controller
              qdd = (u - qd * damp + perturbationForce)/mass; % acceleration
              qd  = qdd * dt + qd; % velocity
              q   = qd * dt + q; % postion
              %w = w + 0.03*randn(n_dim,1);

              D(k,m).traj_y(:,n)   = traj_y; % desired position
              D(k,m).traj_yd(:,n)  = traj_yd; % desired velociy
              D(k,m).h(:,n)   = h;
              D(k,m).G(:,:,n) = G;
              D(k,m).eps(:,n) = eps;
          ...
          %%% If executing on Hardware, reference trajectory and stiffness
          %%% are saved to a file. Actual position of robot (q) does not
          %%% matter because reference trajectory is computed open loop
           if p.HWinLoop    % only wokrs with n_subtrials = 1
              %%% visualization of rollout
              k
              figure
              subplot(3,3,1)
              hold on
              scatter(D(k,m).q(1,:),D(k,m).q(2,:),'r.')
              if p.rep == 1
...
%                   %%% write to file for robot to execute [0 x1 x2 cos(angle/2) sin(angle/2) 0 0]
                    qw = cos(p.offset(4)/2)*cos((D(k,m).traj_y(3,:)+p.offset(5))/2)*cos(p.offset(6)/2) + sin(p.offset(4)/2)*sin((D(k,m).traj_y(3,:)+p.offset(5))/2)*sin(p.offset(6)/2);
                    qx = -cos(p.offset(4)/2)*sin((D(k,m).traj_y(3,:)+p.offset(5))/2)*sin(p.offset(6)/2) + sin(p.offset(4)/2)*cos((D(k,m).traj_y(3,:)+p.offset(5))/2)*cos(p.offset(6)/2);
                    qy = cos(p.offset(4)/2)*sin((D(k,m).traj_y(3,:)+p.offset(5))/2)*cos(p.offset(6)/2) + sin(p.offset(4)/2)*cos((D(k,m).traj_y(3,:)+p.offset(5))/2)*sin(p.offset(6)/2);
                    qz = cos(p.offset(4)/2)*cos((D(k,m).traj_y(3,:)+p.offset(5))/2)*sin(p.offset(6)/2) - sin(p.offset(4)/2)*sin((D(k,m).traj_y(3,:)+p.offset(5))/2)*cos(p.offset(6)/2);
                    rolloutTrajAndImp = [ ones(n_steps_tot,1).*p.offset(1) , D(k,m).traj_y(1,:)'+ p.offset(2) , D(k,m).traj_y(2,:)' + p.offset(3), qx', qy' ,qz',qw', D(k,m).kp(1,:)'];  % [px py pz qx qy qz qw k(1d)], the simulation works in the XZ plane, axis [0 1 0] angle representation is transformed to quaternion
                    save('~/catkin_ws/src/trajectory_publisher/trajectories/new/traj.txt', 'rolloutTrajAndImp','-ascii');
                    %%% chose whether to read result file or discard
                    %%% rollout
                    prompt = 'Waiting for data form hardware execusion. Press y when ready, n to discard rollout.';
                    str = input(prompt,'s');
                    if strcmp(str,'y')
                        %%% read file for actual traj of robot
                        fp = fopen('~/catkin_ws/src/trajectory_publisher/recordings/rec2','r'); %[px py pz qx qy qz qw Fx Fy Fz Tx Ty Tz]
                        if fp == -1,
                          'Cannot open traj from robot file'
                          k = k -1;
                          continue;
                        end
                        count    = 0;
                        while 1
                              ...
                              data = textscan(line,'%f %f %f %f %f %f %f %f %f %f %f %f %f %f'); %[time px py pz qx qy qz qw Fx Fy Fz Tx Ty Tz]
                              D(k,m).real_q(1,count) = data{3} - p.offset(2);
                              D(k,m).real_q(2,count) = data{4} - p.offset(3);
                              D(k,m).real_q(3,count) = asin(2*(data{8}.*data{6} - data{5}.*data{7})) - p.offset(5); % transform to theta from euler sequence 1,2,3
                              D(k,m).real_F(1,count)   = data{9};
                              D(k,m).real_F(2,count)   = data{10};
                              D(k,m).real_F(3,count)   = data{11};
                              D(k,m).real_T(1,count)   = data{12};
                              D(k,m).real_T(2,count)   = data{13};
                              D(k,m).real_T(3,count)   = data{14};
                        end
                        if count < n_steps_tot    % fill missing time steps with last recorded value
                              D(k,m).real_q(1,count:end) = mean(D(k,m).real_q(1,count-5: count-1));
                              D(k,m).real_q(2,count:end) = mean(D(k,m).real_q(2,count-5: count-1));
                              D(k,m).real_q(3,count:end) = mean(D(k,m).real_q(3,count-5: count-1));
                              D(k,m).real_F(1,count:end)   = mean(D(k,m).real_F(1,count-5: count-1));
                              D(k,m).real_F(2,count:end)   =  mean(D(k,m).real_F(2,count-5: count-1));
                              D(k,m).real_F(3,count:end)   = mean(D(k,m).real_F(3,count-5: count-1));
                              D(k,m).real_T(1,count:end)   = mean(D(k,m).real_T(1,count-5: count-1));
                              D(k,m).real_T(2,count:end)   =  mean(D(k,m).real_T(2,count-5: count-1));
                              D(k,m).real_T(3,count:end)   = mean(D(k,m).real_T(3,count-5: count-1));
                        end
                    else
                        k = k -1; % this will redo a new roll-out as replacement and overwrite previous one
                    end
                    delete('~/catkin_ws/src/trajectory_publisher/recordings/rec2'); % by deleting the file we make sure the next recording file will be called rec2.txt
              elseif p.rep ==1
                    error('bad updated policy, exiting') %how to deal with this case
              else
                    k = k-1;  % this will redo a new roll-out as replacement and overwrite previous one
              end
          end
    ...
    if showRollouts && p.rep>1 && ~p.disable_plotting
        for k = 1:p.rep
            ...
            allRolloutTrajs(k,:,3) = D(k,:).q(3,:);
        end
        
        figure
        plot(allRolloutTrajs(:,:,1)',allRolloutTrajs(:,:,2)','.')
        figure
        plot(allRolloutTrajs(:,:,1)',allRolloutTrajs(:,:,3)','.')
...
global n_out;
    ...
    if n_in < n_dim   % discard unused dim inputs. Needs to be modified for SEDS
        nonInputDim = n_dim-n_in;
        GMM.Mu(n_in+1:n_in+nonInputDim,:) = [];
        GMM.Sigma(n_in+1:n_in+nonInputDim,:,:) = [];
        GMM.Sigma(:,n_in+1:n_in+nonInputDim,:) = [];
    end
...
%dt = p.dt;
%T  = (1:length(D(1).q))'*dt;
T_real = p.dt:p.dt:p.duration;
T_tot = p.dt:p.dt:p.duration+p.duration_convergence;
...
for j=1:n_dim;
  ...
  % (and reorder)
%   subplot(2*n_dim_pos,5,(j-1)*10+3);
%   
%   for k=1:p.rep,
%       TT(:,k)=D(k).dmp(j).ydd;
%   end
...
%   ylabel(sprintf('ydd_%d',j));
  ...
  plot(T_tot,D_eval(1).kp(j,:),'Color',gray,'LineWidth',2);
  hold on;
  plot(T_tot,TT);
  hold off;
...
%   subplot(n_dim_pos+1,4, n_dim_pos*4+3);
% 
%   bar(GMR.theta);
%   ylabel('theta');
%   axis('tight');
...
if n_dim_pos == 2;
    GMRreduced = GMR;
    GMRreduced.A = GMR.A(1:n_dim_pos,:,:);    % get rid of angle and kp dimension for using streamlines
    GMRreduced.b = GMR.b(1:n_dim_pos,:,:);
    currentAxis = axis;
...
if n_dim_pos ==2
    my_plotGMM(GMR.muInput(1:2,:), GMR.sigmaInput(1:2,1:2,:), 'r', 1);
...
if n_dim_angle == 1;
    figure
    hold on
    plot(D_eval(1,m).q(n_dim_pos+1,1:p.duration/p.dt),'b.');
    plot(D_eval(1,m).traj_y(n_dim_pos+1,1:p.duration/p.dt),'r.');
    hold off
    figure
    hold on
    plot(D_eval(1,m).q(1,1:p.duration/p.dt),D_eval(1,m).q(n_dim_pos+1,1:p.duration/p.dt),'b.');
    plot(D_eval(1,m).traj_y(1,1:p.duration/p.dt),D_eval(1,m).traj_y(n_dim_pos+1,1:p.duration/p.dt),'r.');
    hold off
end
