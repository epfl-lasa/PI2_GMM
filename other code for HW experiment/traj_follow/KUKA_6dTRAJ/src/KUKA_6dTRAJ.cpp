...
void KUKA_6dTRAJ::measuredForceCallback(const geometry_msgs::WrenchStamped wrenchMsg){
    latestForceMeasurement(0) = wrenchMsg.wrench.force.x;
    latestForceMeasurement(1) = wrenchMsg.wrench.force.y;
    latestForceMeasurement(2) = wrenchMsg.wrench.force.z;
}
...
//    qInterp = new quat;
     ...
     wrenchSub = nh->subscribe("/comp_wrenchBase_pub",100,&KUKA_6dTRAJ::measuredForceCallback,this);
...
RobotInterface::Status KUKA_6dTRAJ::RobotUpdateCore(){
    ...
    case EXECUTE:
	    ...
	    kTempRos = &(trajectoryQueue.front().stiffs[currRow]);
	  //  cout <<  *kTempRos << "\n";
            // set the cartcommand
            ...
            RLStiffness.Resize(6);
	    for(int i=0;i<3;i++){
      		RLStiffness(i) = *kTempRos;
      		RLStiffness(3+i) = *kTempRos/10;
   	    }
            ...
            mLWRRobot->SetCartCommand(cartCommand,cartCommandOrient);
	  //  RLStiffness.Print();
          //  latestForceMeasurement.Print();
            if(latestForceMeasurement.Norm() < 50)
	    {
	        mLWRRobot->SetCartStiffness(RLStiffness);
	    }
	    else
	    {
                cout << "Forces higher than limit, setting stiffness to min value" << "\n";
            	mLWRRobot->SetCartStiffness(minStiffness);
            }
